...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(345.0, 150.0),
                rg.Point(322.5, 189.0),
                rg.Point(277.5, 189.0),
                rg.Point(255.0, 150.0),
                rg.Point(277.5, 111.0),
                rg.Point(322.5, 111.0)]
    circle = rg.Circle(rg.Point(300, 150), 45)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 2:  8 orange dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 150), 175)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  25 slices, thin (thickness=2) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 2
    pizza(window, circle, 25, 'black', 2)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(seq)):
        line = rg.Line(circle.center, seq[k])
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = ('FANCY POLYGON test 4:  30 blue lines on lime green circle,'
             + ' hops = 5.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'lime green'
    fancy_polygon(window, circle, 30, 5, 'blue', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(seq)):
        line = rg.Line(seq[k], seq[(k+hops_to_next_point) % number_of_lines])
        line.arrow = 'last'
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
